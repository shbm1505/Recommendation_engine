...
object tfidf_test {
  def main(args: Array[String]) {
    ...
    val txn_ucic = transactionDF.join(accountDF, transactionDF("account_no") === accountDF("account_no"), "left")
    txn_ucic.registerTempTable("txn_ucic")

    val txncount = sqlContext.sql("select count(*) from txn_ucic")
    txncount.show()

    //val txnnull = sqlContext.sql("select count(*) from txn_ucic where ucic is null")
    //txnnull.show()
    /*
    ...
    val tfDF = sqlContext.sql("select ucic, range_subcat, count(range_subcat) as tf from txn_merchantcat group by range_subcat, ucic")
    ...
    val tfidfFinal = sqlContext.sql("select ucic, range_subcat, tf,doc_freq, idf, tf*idf as tf_idfscore from tfidfDF where ucic is not null")
    tfidfFinal.show()
    tfidfFinal.registerTempTable("tfidfFinal")

    val tfidfnullc = sqlContext.sql("select count(*) from tfidfDF where ucic is null")
    tfidfnullc.show()
