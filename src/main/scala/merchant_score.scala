...
object merchant_score {
  ...
  def main(args: Array[String]) {
    ...
    val merchant_cat = sqlContext.sql("select cleaned_merchant_category, cleaned_merchant_name, count(*) as merchant_cat_txncount, max(date_of_txn) as lasttxnday,datediff(now(),max(date_of_txn)) as days_from_lasttxn from txn_ucic where cleaned_merchant_category is not null group by cleaned_merchant_category,cleaned_merchant_name ")

    merchant_cat.registerTempTable("merchant_cat")

    val freq_cat = List("Dining & Bar", "Entertainment", "Groceries & Supermar", "Utilities", "Petrol")

    val merchant_cat_recency = merchant_cat.withColumn("Recency", when($"cleaned_merchant_category" isin (freq_cat: _*), exp(-($"days_from_lasttxn") / 14)).otherwise(exp(-($"days_from_lasttxn") / 60)))
    //merchant_cat_recency.show()
    merchant_cat_recency.registerTempTable("merchant_cat_recency")


    val merchant_cat_score = merchant_cat_recency.select($"cleaned_merchant_category", $"cleaned_merchant_name", ($"merchant_cat_txncount" * $"Recency") as "merchant_weight")
    ...
    val schema_string = "id,cleaned_merchant_category,cleaned_merchant_name,merchant_weight,quartile"
    ...
    for (i <- distinctcat) {
      ...
      val dfsorted = dfWithID.sort($"merchant_weight".desc)
      val dfquartile = dfsorted.withColumn("quartile", when(($"id" < cutoff), 1)
    ...
    dfcombined.write
      ...
      .options(Map("table" -> "category_merchant_weight2", "keyspace" -> "goals101"))
      .save()
